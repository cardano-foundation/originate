        ...
        <div id="kc-info-message">
            <div style="text-align: center">
                <#if message.summary==msg('confirmExecutionOfActions')>
                    <p class="instruction text_center">${message.summary}<#if requiredActions??>:<div><#list requiredActions> <b><#items as reqActionItem>${kcSanitize(msg("requiredAction.${reqActionItem}"))?no_esc}<#sep>, </#items></b></#list><#else></#if></p></div>
                <#elseif message.summary==msg('accountUpdatedMessage')>
                    <p class="instruction text_center">${message.summary}<#if requiredActions??><#list requiredActions>: <b><#items as reqActionItem>${kcSanitize(msg("requiredAction.${reqActionItem}"))?no_esc}<#sep>, </#items></b></#list><#else></#if></p>
                <#else >
                    <p class="instruction text_center">${message.summary}<#if requiredActions??><#list requiredActions>: <b><#items as reqActionItem>${kcSanitize(msg("requiredAction.${reqActionItem}"))?no_esc}<#sep>, </#items></b></#list><#else></#if></p>
                </#if>
            </div>
            ...
            <#if skipLink??>
                    <input readonly="readonly"  style="cursor: pointer; text-align: center;" tabindex="4" onclick="changUrl('${properties.url_app}')" class="${properties.kcButtonClass!} ${properties.kcButtonPrimaryClass!} ${properties.kcButtonBlockClass!} ${properties.kcButtonLargeClass!}" value="${kcSanitize(msg("backToApplication"))?no_esc}"></input>
            <#else>

...
<#--<script type="text/javascript" src="${url.resourcesPath}/js/index.js"></script>-->
                        ...
                        <div class="${properties.kcFormGroupClass!}">
                            <#if usernameEditDisabled??>
                                ...
                                <input tabindex="1" id="username" class="${properties.kcInputClass!}" name="username"
                                       value="${(login.username!'')}" type="email" disabled
                                       placeholder="<#if !realm.loginWithEmailAllowed>${msg("email")}<#elseif !realm.registrationEmailAsUsername>${msg("usernameOrEmail")}<#else>${msg("email")}</#if>"/>
                            <#else>
                                <input tabindex="1" id="username" class="${properties.kcInputClass!}" name="username"
                                       value="${(login.username!'')}" type="email" autocomplete="off"
                                       placeholder="<#if !realm.loginWithEmailAllowed>${msg("email")}<#elseif !realm.registrationEmailAsUsername>${msg("usernameOrEmail")}<#else>${msg("email")}</#if>"/>
                            </#if>
                        </div>
    ...
    <#elseif section = "info" >
        <#if realm.password && realm.registrationAllowed && !registrationDisabled??>
            <div id="kc-registration">
                <span>${msg("noAccount")} xxxx <a tabindex="6" href="${url.registrationUrl}">${msg("doRegister")}</a></span>
            </div>
        </#if>
    </#if>
